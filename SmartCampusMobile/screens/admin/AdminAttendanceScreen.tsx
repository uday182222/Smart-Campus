/**
 * Admin — Mark attendance for any class (same P/A/L flow as teacher).
 */
...
export default function AdminAttendanceScreen() {
    ...
    try {
      ...
      const attendanceMap = new Map(
        attendanceList.map((a: any) => [
          a.studentId,
          a.status === 'present' || a.status === 'absent' || a.status === 'late' ? a.status : 'not_marked',
        ])
      );
  ...
  const saveAttendance = async () => {
    ...
    if (unmarked > 0) {
      Alert.alert('Incomplete', `${unmarked} student(s) not marked. Mark all as present and save?`, [
