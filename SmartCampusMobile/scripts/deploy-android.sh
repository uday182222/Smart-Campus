...
# Android Deployment Script for Smart Campus
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Smart Campus - Android Deployment${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo ""
echo -e "${BLUE}Building Android app with profile: ${BUILD_PROFILE}${NC}"
echo ""
...
if [ $? -eq 0 ]; then
    ...
    if [ "$BUILD_PROFILE" = "production" ]; then
        read -p "Do you want to submit to Google Play now? (y/n): " submit_choice
        ...
        if [ "$submit_choice" = "y" ] || [ "$submit_choice" = "Y" ]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                echo -e "${YELLOW}Check Play Console for review status${NC}"
            fi
        else
            echo ""
        ...
        echo -e "${YELLOW}APK build completed. Download from Expo dashboard.${NC}"
    fi
else
    echo ""
...
if [ "$BUILD_PROFILE" = "production" ]; then
    echo "1. Check Google Play Console for build status"
    echo "2. Complete store listing if not done"
    echo "3. Fill content rating and data safety"
    echo "4. Submit for review"
    echo "5. Monitor review status"
elif [ "$BUILD_PROFILE" = "preview" ]; then
    echo "1. Download APK from Expo dashboard"
    echo "2. Distribute to internal testers"
    echo "3. Collect feedback"
    echo "4. Fix issues before production"
else
    echo "1. Download the build from Expo dashboard"
    echo "2. Install on test devices via USB"
